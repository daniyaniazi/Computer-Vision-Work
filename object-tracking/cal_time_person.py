    ...
    while True:
        ...
        if ret != None:
            ...
            for (objectId, bbox) in objects.items():
                ...
                # checking id for tracking
                if objectId not in (object_id_list):
                    object_id_list.append(objectId)
                    dtime[objectId] = datetime.datetime.now()
                    dwell_time[objectId] = 0
                else:
                    # if person already there
                    current_time = datetime.datetime.now()
                    old_time = dtime[objectId]  # check old time presence
                    time_difference = current_time - old_time  # cal difference

                    # initialize new current time
                    dtime[objectId] == datetime.datetime.now()
                    # add the seconds spend
                    sec = time_difference.total_seconds()
                    dwell_time[objectId] += sec

                cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 0, 255), 2)
                text = "Person No: {} Spend : {} secs".format(
                    objectId, int(dwell_time[objectId]))
